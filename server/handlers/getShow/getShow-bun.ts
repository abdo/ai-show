// ============================================================================
// getShow-bun.ts - Complete Self-Contained Bun Server for AI Show Generation
// ============================================================================
// TypeScript version for Railway deployment using Bun runtime
// Contains ALL logic: prompts, services, server - single file deployment
...
function assignAvatarsAndColors(characters: Character[]): Character[] {
  ...
  return characters.map((char, index) => {
    ...
    if (normalizedGender === "male") {
      ...
      console.warn(`Invalid gender value "${rawGender}" for character ${char.name}`);
...
Bun.serve({
  ...
  async fetch(req: Request): Promise<Response | undefined> {
    ...
    if (url.pathname === '/health') {
      return new Response(JSON.stringify({ status: 'ok', service: 'ai-show-server-bun' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }

    // Main getShow endpoint
    if (url.pathname === '/getShow' && req.method === 'POST') {
      ...
      try {
        ...
        const { userInput, userName, mode } = body as { userInput?: string; userName?: string; mode?: 'conversation' | 'story' };
        ...
        console.log('[getShow] Request received', { userInput, userName, mode });
        ...
        return new Response(JSON.stringify({ story, audioMap }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        });
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
...
// Make this file a proper ES module to avoid TypeScript errors
export { };
